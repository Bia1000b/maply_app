...
class PlaceSuggestion {
  ...
  PlaceSuggestion({required this.description, this.placeId, this.lat, this.lng});
}
...
class PlaceAutocompleteService {
  ...
  Future<List<PlaceSuggestion>> fetch(String input, {String countryCode = ''}) async {
    ...
    if (googleApiKey != null && googleApiKey!.isNotEmpty) {
      try {
        ...
        final url = Uri.https('maps.googleapis.com', '/maps/api/place/autocomplete/json', {
          ...
          'components': countryCode.isNotEmpty ? 'country:$countryCode' : null,
        ...
        if (resp.statusCode == 200) {
          ...
          if (j['status'] == 'OK') {
            final preds = (j['predictions'] as List)
                .map((p) => PlaceSuggestion(description: p['description'], placeId: p['place_id']))
                .toList();
            return preds;
          }
        }
    ...
    try {
      final url = Uri.parse('https://nominatim.openstreetmap.org/search?q=${Uri.encodeComponent(input)}&format=json&limit=6&addressdetails=0');
      final resp = await http.get(url, headers: {'User-Agent': 'maply_app/1.0'});
      if (resp.statusCode == 200) {
        final j = json.decode(resp.body) as List;
        return j.map((e) => PlaceSuggestion(
